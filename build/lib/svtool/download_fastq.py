...
import re
import argparse
from .common_utils import check_file_exists, run_command, initialize_logger

def check_fastq_integrity(logger, file_local, ftp_folder):
    """
    ...
    cmd_ftp = f"curl {ftp_folder}|awk '{{if ($NF==\"{file_name}\") print $5}}'"
    logger.debug(cmd_ftp)
    size_ebi = os.popen(cmd_ftp).read().strip()

    if size_local != size_ebi:
        logger.warning(f'{file_local}: incomplete fastq file. EBI size={size_ebi}, local size={size_local}')
        return 0
    else:
        return 1
...
def download(logger, srr_raw, is_single_end, output_folder): 
    ...
    if res == 1:   # fail to get fastq file on EBI server
        ...
        run_command(logger, f'fastq-dump --gzip --helicos {split} --origfmt -O {output_folder} {srr}')
    else:
        # first check if the file already exist
        for ifl in res:
            ifl_local = f'{output_folder}/{ifl}'
            if os.path.isfile(ifl_local) and check_fastq_integrity(logger, ifl_local, ftp_folder):
        ...
        if len(res_new) == 0:
            logger.info("All files has already been download and complete, if you want to re-download it, delete files and try again")
            return 0
        else:
            # download the file
            ...
            run_command(logger, f"cd {output_folder}; parallel 'wget -c {{}}' ::: {url} ")

            # verify the downloaded file
            fail = 0
            for ifl in res_new:
                ifl_local = f'{output_folder}/{ifl}'
                valid = check_fastq_integrity(logger, ifl_local, ftp_folder)
                if valid:
...
def add_download_parser(ps, notDebug):
    ps.add_argument("-i", "--input", help="Input SRR ID", required=notDebug)
    ps.add_argument("-s", "--is_single_end", action='store_true', help="Is the sample single-end data?")
    ps.add_argument("-o", "--output", help="Output folder", required=notDebug)
...
def main():
    ...
    add_download_parser(ps, NOT_DEBUG)
    ...
    logger = initialize_logger(os.path.join(args.output, args.input + ".log"), "svtool")
